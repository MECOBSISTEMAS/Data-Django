...
def gerar_arquivo_pdf(request, context):
    with tempfile.NamedTemporaryFile(delete=False) as tmp:
        pdf = canvas.Canvas(tmp.name, initialFontSize=12, initialFontName='Helvetica', pagesize=(595, 842))
        ...
        pdf.drawString(100, 450, request.session.get('valores_pagos_honorarios'))
        pdf.drawString(100, 300, request.session.get('repasses_geral_descontado'))
        pdf.save()
